...
**Author:** [Kirk Bansak](https://www.kirkbansak.com/) (UCSD); Eddie Yang (UCSD); 
...
Suppose we are interested in the power of an experiment, in which exactly half of the experimental subjects will be assigned to treatment and the other half to control. Further suppose the sample size is 3000, the compliance rate is 0.5 and the treatment effect size is 0.25. Without making the ordered-means assumption, we can compute the power of this experiment by the following:
...
In addition to the output that is printed in the console, `powerLATE()` returns both the list of input parameters and output parameters as invisible objects. We can get thses values by typing
```{r}
